...
assert.ok(Number.isFinite(summary.deterministic_pass_rate), "deterministic pass rate is numeric");
assert.ok(Number.isFinite(summary.source_faithful_pass_rate), "source-faithful pass rate is numeric");
assert.ok(Number.isFinite(summary.average_audit_score), "average audit score is numeric");
...
assert.strictEqual(summary.ai_fallback_implemented, true, "AI fallback implementation status is explicit");
assert.strictEqual(typeof summary.ai_key_available, "boolean", "AI key availability is explicit");
assert.ok(summary.ai_usage && typeof summary.ai_usage === "object", "AI usage summary is present");
assert.ok(Number.isFinite(summary.ai_usage.spent_usd), "AI spend is numeric");
...
assert.ok(Array.isArray(summary.partial_cases), "partial cases list is present");
...
for (const row of results.filter((r) => r.import_status !== "skipped")) {
  assert.ok(row.source_audit && typeof row.source_audit === "object", `${row.id} has source audit`);
  assert.ok(Number.isFinite(row.source_audit_score), `${row.id} has source audit score`);
  assert.strictEqual(typeof row.review_needed, "boolean", `${row.id} has review-needed flag`);
}
