...
7. ~~Tag-based filtering / smart collections (e.g. "Copycat").~~ **Done (tag system v1).**
   ...
   - **Smart collections — Done.** The Library dashboard now shows a "Collections" row of curated cards (Copycat, Weeknight, Quick, Meal Prep, Comfort Food, One-Pot, Air Fryer, Slow Cooker, Sheet Pan, Healthy, Budget-Friendly, Holiday, Party, Kid-Friendly). Each is built only from tags the user actually has (no empty shelves), sorted by recipe count, capped to 6, and taps through to the existing tag filter. Pure UI over existing tag data — no schema/API/credit change.
   - **Remaining future enhancement:** optionally letting users rename/merge their own tags, and user-defined/saved custom collections.
